...
function getSuggestedQuestions(packageData, customerData, chatHistory, lastMessage) {
  ...
  } else if (lastMsg.includes('open') || lastMsg.includes('start') || lastMsg.includes('rapport')) {
    questions.push("What's their main concern?");
    questions.push("Qualifying questions to ask");
    questions.push("Find their motivation");
  } else if (lastMsg.includes('trust') || lastMsg.includes('reliability') || lastMsg.includes('cowboys')) {
    questions.push("RACQ's track record facts");
    questions.push("Share horror stories (tactfully)");
    questions.push("Our warranty vs theirs");
  } else {
